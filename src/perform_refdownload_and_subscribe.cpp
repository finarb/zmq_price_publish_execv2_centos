...
/**                                                    **/
/** retrieving referential (i.e. static) data and subscribe **/
/**                                                    **/
/** QuantHouse 2011                                    **/
/********************************************************/
...
namespace RefDownloadAndSubscribeSample
{
...
public:
	...
	FEEDOS_GEN_DECL_INTERFACE_REFERENTIAL_DownloadAndSubscribe_streaming
};


//![snippet_ref_download_started]
void RefDownloadPrinter::response_REFERENTIAL_DownloadAndSubscribe_Started()
{
	if (m_opt_verbose)
	{
...
void RefDownloadPrinter::response_REFERENTIAL_DownloadAndSubscribe_Failed(
		ReturnCode rc)
{
...
void RefDownloadPrinter::aborted_REFERENTIAL_DownloadAndSubscribe(
		ReturnCode rc)
{
	if (RC_OK != rc)
	{
		...
		ListOfFOSInstrumentCode const & inInternalCodes)
{
	...
	show_progress();
	for (ListOfFOSInstrumentCode::const_iterator it =inInternalCodes.begin();
			it != inInternalCodes.end(); ++it)
	{
...
//![snippet_ref_download_realtime_begin]
void RefDownloadPrinter::notif_RealtimeBegin()
{
	if (m_opt_verbose)
	{
		std::cerr << "realtime events..." << std::endl;
	}
}
//![snippet_ref_download_realtime_begin]

//![snippet_ref_download_marker_ts]
void RefDownloadPrinter::notif_MarkerTimestamp(
		Timestamp const & inMarkerTimestamp)
{
	if (m_opt_verbose)
	{
		std::cerr << "marker timestamp=" << inMarkerTimestamp << std::endl;
	}
}
//![snippet_ref_download_marker_ts]

//![snippet_ref_download_vts]
void RefDownloadPrinter::notif_VariablePriceIncrementTables(
		ListOfVariableIncrementPriceBandTable const & inTables)
{
	if (m_opt_verbose)
	{
		std::cerr << "variable price increment tables received" << std::endl;
		dump_tick_size_tables(inTables);
	}
}
//![snippet_ref_download_vts]

//![snippet_ref_download_tradecond]
void RefDownloadPrinter::notif_TradeConditionsDictionary(
		ListOfTradeConditionsDictionaryEntry const & inEntries)
{
	if (m_opt_verbose)
	{
		std::cerr << "trade conditions dictionary received" << std::endl;
		dump_trade_conditions(inEntries);
	}
}
//![snippet_ref_download_tradecond]

//![snippet_ref_download_meta]
void RefDownloadPrinter::notif_MetaData(
		ListOfTagDeclaration const & inTags,
		ListOfEnumTypeDeclaration const & inEnums,
		ListOfString const & inProviders)
{
	if (m_opt_verbose)
	{
		std::cerr << "metadata received" << std::endl;
		dump_metadata(inTags, inEnums, inProviders);
	}
}
//![snippet_ref_download_meta]
...
void RefDownloadPrinter::dump_instr_data_one_line_PRETTY (InstrumentCharacteristics const & instr_data)
{
	...
	if (0!=maturity_year) {
		if (0!=maturity_month) {
			if (0!=maturity_day) {
				sprintf (maturity_str, "%04i-%02i-%02i", (int)maturity_year, (int)maturity_month, (int)maturity_day);
			} else {
				sprintf (maturity_str, "%04i-%02i", (int)maturity_year, (int)maturity_month);
			}
		} else {
...
void RefDownloadPrinter::show_progress ()
{
	if (m_opt_show_progress) {
		std::cerr << "TOTAL: deleted="<<m_counter_nb_deleted<<" created="<<m_counter_nb_created<<" modified="<<m_counter_nb_modified<<" \r" << std::flush;
	}
}

} // namespace DownloadAndSubscribeSample



/** \cond refguide
 * To perform a referential download and subscribe in FeedOS API C++, you need to implement an
 * interface and several callbacks that handle the request and response, then adjust the
 * returned data's attributes and filter the result, as described in the procedure below.
 *
 * This sample prints the instruments belonging to the referential database along with
 * their tags on the standard output.
 *
 * <ol><li>Implement the \c FeedOS::INTERFACE_REFERENTIAL_DownloadAndSubscribe_streaming
 * interface whose virtual methods are called when the download is performed:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_subclass
 * 
 * <ol><li>Implement the started callback that is called when the server acknowledges the query,
 * as shown below:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_started
 *
 * <ol><li>Implement the failed callback to be called if there is an error during the query:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_failed
 *
 ...
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_aborted
 *
 ...
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_branchbegin
 *
 * <ol><li>Implement the created instruments callback to be called for each created instrument:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_created
 *
 * <ol><li>Implement the modified instruments callback to be called for each modified instrument:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_modified
 *
 * <ol><li>Implement the deleted instruments callback to be called for each deleted instrument,
 * as shown below:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_deleted
 *
 * <ol><li>Implement the real time begin callback to be called when the referential download is complete
 * and the real-time notifications start:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_realtime_begin
 *
 * <ol><li>Implement the marker timestamp callback to be called whenever an update is performed
 * on the referential (subscription):</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_marker_ts
 *
 * <ol><li>Implement the variable tick size tables callback to be called when the variable tick size
 * tables are updated:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_vts
 *
 * <ol><li>Implement the trade conditions callback to be called when the trade conditions'
 * dictionary is updated:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_tradecond
 *
 * <ol><li>Implement the metadata callback to be called when the metadata are updated:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_meta
 *
 * <ol><li>Customize the instruments to be retrieved, by adjusting the following parameters:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_params
 *
 ...
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_custom
 *
 * <ol><li>Customize the attributes to be downloaded:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_attr
 *
 * <ol><li>Filter the attributes to be downloaded:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_attr_filter
 *
 * <ol><li>Register the object whose callbacks will be called, as shown below:</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_reg
 *
 * <ol><li>Perform the request. The request is synchronous: the process waits for the server to reply
 * and blocks the calling thread until suspension.</li></ol>
 * \snippet perform_refdownload_and_subscribe.cpp snippet_ref_download_req
 *
 * \endcond
 */
void perform_refdownload_and_subscribe (FeedOS::API::Connection & connection, int argc, char * argv[]) 
{
	FEEDOS_SCOPER (User, "perform_refdownload_and_subscribe");	

	//![snippet_ref_download_params]
	// if empty, then all the branches are retrieved
	ListOfMarketBranchId		param_TargetBranches;
	// if empty, then no filtering is performed
	...
	//
	// [optional] list of attributs to filters
	//
	// Type=Future
	param_FilteringAttributes.push_back(ReferentialAttribute_SecurityType("FUT"));
	//	AND expiry in 2008
	param_FilteringAttributes.push_back(ReferentialAttribute_MaturityYear(2012));
	//![snippet_ref_download_attr_filter]
#endif
	...
	RefDownloadAndSubscribeSample::RefDownloadPrinter printer(
			std::cout, 		// output stream
			opt_verbose,	// "VERBOSE"
			opt_verbose,	// "SHOW PROGRESS"
			opt_raw);		// "RAW MODE"

	API_REFERENTIAL_DownloadAndSubscribe_delegate req(printer);
	//![snippet_ref_download_reg]
	...
	req.sync_stop(); // stop subscription
	//![snippet_ref_download_req]
}
